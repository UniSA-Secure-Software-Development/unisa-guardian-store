...
export class SearchResultComponent implements OnDestroy, AfterViewInit {
  ...
  trustProductDescription (tableData: any[]) { // vuln-code-snippet neutral-line restfulXssChallenge
    for (let i = 0; i < tableData.length; i++) { // vuln-code-snippet neutral-line restfulXssChallenge
      tableData[i].description = SecurityHelper.sanitizeAndEncodeInputForHTML(this.sanitizer, tableData[i].description) // vuln-code-snippet vuln-line restfulXssChallenge
  ...
  filterTable () {
    ...
    if (queryParam) {
      ...
      this.searchValue = SecurityHelper.sanitizeAndEncodeInputForHTML(this.sanitizer, queryParam) // vuln-code-snippet vuln-line localXssChallenge xssBonusChallenge
