...
module.exports = function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    // Create parameters
    const emailSafe = req.body.email
    const passwordSafe = security.hash(req.body.password)
    // Create parameterized query
    models.sequelize.query('SELECT * FROM Users WHERE email = :emailSafe AND password = :passwordSafe AND deletedAt IS NULL', { model: UserModel, plain: true, replacements: { emailSafe, passwordSafe } }) // vuln-code-snippet vuln-line loginAdminChallenge loginBenderChallenge loginJimChallenge
