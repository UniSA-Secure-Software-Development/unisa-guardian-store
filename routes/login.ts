...
module.exports = function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const email = req.body.email || '' // Get email from request or set it as an empty string
    const hashedPassword = security.hash(req.body.password || '') // Hash the password from the request or set it as an empty string

    models.sequelize.query('SELECT * FROM Users WHERE email = :email AND password = :password AND deletedAt IS NULL', {
      model: UserModel,
      plain: true,
      replacements: { email, password: hashedPassword } // Replace placeholders with sanitized values
    })
