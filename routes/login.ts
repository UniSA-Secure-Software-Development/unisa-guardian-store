...
module.exports = function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const pswd = security.hash(req.body.password || '')
    models.sequelize.query('SELECT * FROM Users WHERE email = $1 AND password = $2 AND deletedAt IS NULL',
      { bind: [email, pswd], type: models.sequelize.QueryTypes.SELECT, model: UserModel, plain: true }) // vuln-code-snippet vuln-line loginAdminChallenge loginBenderChallenge loginJimChallenge
