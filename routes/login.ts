...
module.exports = function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const passwordHash: string = security.hash(req.body.password || '')
    verifyPreLoginChallenges(req) // vuln-code-snippet hide-line
    models.sequelize.query('SELECT * FROM Users WHERE email = :email AND password = :passwordHash AND deletedAt IS NULL', {
      replacements: {
        email,
        passwordHash
      },
