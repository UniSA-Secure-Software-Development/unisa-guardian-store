...
module.exports = function searchProducts () {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const escaped = raw.replace(/[%_]/g, '\\$&') // Escape % and _ for LIKE query
    const likePattern = `%${escaped}%`
    models.sequelize.query(
      'SELECT * FROM Products WHERE ((name LIKE :like OR description LIKE :like) AND deletedAt IS NULL) ORDER BY name',
      {
        replacements: { like: likePattern }
      }
