...
module.exports = function searchProducts () {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (criteria.length > 200) criteria = criteria.substring(0, 200)
    if (!/^[\w\s\-.,&()!?:'"]{0,200}$/.test(criteria)) {
      return res.status(400).json({ error: 'Invalid search input' })
    }
    models.sequelize.query('SELECT * FROM Products WHERE ((name LIKE :c OR description LIKE :c) AND deletedAt IS NULL) ORDER BY name',
      {
