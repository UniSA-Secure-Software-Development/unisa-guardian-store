...
module.exports = function login() {
  function afterLogin(user, res, next) {
    ...
    basket_1.BasketModel.findOrCreate({ where: { UserId: user.data.id } })
      .then(([basket]) => {
        ...
        user.bid = basket.id; // keed track of original basket
  ...
  return (req, res, next) => {
    ...
    models.sequelize.query('SELECT * FROM Users WHERE email = ? AND password = ? AND deletedAt IS NULL', { replacements: [`${req.body.email || ''}`, `${security.hash(req.body.password || '')}`], model: user_1.UserModel, plain: true }) // vuln-code-snippet vuln-line loginAdminChallenge loginBenderChallenge loginJimChallenge
